...
export default {
  ...
  setup() {
    ...
    const onAvatarUpload = (file) => {
      ...
      // The AvatarCanvas component handles the upload via ref
      // We need to trigger it through the canvas component
      if (avatarCanvasRef.value && avatarCanvasRef.value.uploadAvatar) {
        avatarCanvasRef.value.uploadAvatar(file)
      }
    }

    const avatarCanvasRef = ref(null)
